...
class Paradas_bsSeeder extends Seeder
{
    private $PARADAS_B_JSON = '
    {
        "paradas_bs": [
            {
              "name_ref": "11",
    ...
    public function run()
    {
        $paradas = json_decode($this->PARADAS_B_JSON)->paradas_bs;

        foreach ($paradas as $parada){
            DB::table('paradas_bs')->insert([
                'name_ref'=>$parada->name_ref,
                'coordenada'=>$parada->coordenada,
                'img_ref'=>$parada->img_ref,
            ]);
        }
    }
}
